...
const trackingBoost=preset==='tracking'?2.05:1;
const receptionBoost=preset==='reception'?1.55:1;
const frictionBreak=preset==='friction'?0.62:1;
const arrivalBoost=preset==='arrival'?1.8:1;
const heartBoost=preset==='heart_memory'?1.45:1;
const maxFilaments=preset==='arrival'?Math.floor(engine.maxFilaments*0.75):engine.maxFilaments;
const rate=(0.003+s.tracking*0.036+s.reception*0.026+s.mixed_weather*0.024+(s.arrival||0)*0.02+s.heart_memory*0.012)*restScale*trackingBoost*receptionBoost*frictionBreak*arrivalBoost*heartBoost;
...
const minD=preset==='friction'?0.2:0.11,maxD=preset==='reception'?1.1:(preset==='tracking'?0.98:0.9);
...
let alpha=(1-d/1.05)*(0.06+s.tracking*0.38+s.reception*0.24+s.heart_memory*0.22+s.arrival*0.14)*(0.62+0.34*Math.sin(t+i));
if(preset==='friction'&&Math.random()<0.28)alpha*=0.35;
if(preset==='heart_memory')alpha*=1.35;
if(preset==='arrival')alpha*=1.2;
